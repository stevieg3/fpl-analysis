...
def clean_raw_scraped_data(raw_data_path, write_to_csv=False, new_csv_name=None):
    ...
    raw_df["team_goals"] = np.where(
        raw_df["home_or_away"] == "H", raw_df["goals1"], raw_df["goals2"]
    )
    raw_df["opposition_goals"] = np.where(
        raw_df["home_or_away"] == "H", raw_df["goals2"], raw_df["goals1"]
    )
...
def main():
    os.chdir("../..")
    clean_raw_scraped_data(
        "data/raw/FPL 2017:18 player stats.xlsx",
        write_to_csv=True,
        new_csv_name="clean_scraped_2017_18",
    )

    clean_raw_scraped_data(
        "data/raw/2018_19_current_season_data.csv",
        write_to_csv=True,
        new_csv_name="clean_scraped_2018_19",
    )
